...
class CajaScreenState extends State<CajaScreen>
  ...
  Future<void> _cargarVentas() async {
    ...
    try {
      ...
      List<Venta> ventasCargadas =
          await _databaseService.getVentasPorRangoFechas(fechaInicio, fechaFin);
  ...
  void _aplicarFiltros({String? filtroCategoria}) {
    ...
    _productosFiltrados = _productos.where((producto) {
      bool cumpleBusqueda = producto.nombre.toLowerCase().contains(busqueda) ||
          producto.codigoBarras.toLowerCase().contains(busqueda);

      bool cumpleCategoria =
          categoria == 'Todas' || producto.categoria == categoria;

      bool cumplePrecio = producto.precioVenta >= _filtroPrecioMin &&
  ...
  Future<void> _procesarVentas(BuildContext? dialogContext) async {
    ...
    try {
      ...
      final ventasCasuales =
          _carrito.where((item) => item.esVentaCasual).toList();
      for (final item in ventasCasuales) {
        ...
        final referenciaPago = (_metodoPagoSeleccionado == 'Transferencia' ||
                _metodoPagoSeleccionado == 'Tarjeta de Crédito' ||
                _metodoPagoSeleccionado == 'Tarjeta de Débito')
